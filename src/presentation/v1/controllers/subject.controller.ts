...
import { SectionSubjectsDto } from "../dto/subject-plan-response.dto";
...
export class SubjectController {
  ...
  @ApiQuery({
    name: "plan",
    description: "Plan identifier (e.g. S10-Rev23)",
    required: true,
  })
  @ApiResponse({
    status: 200,
    description: "Subjects organized by section, year, and semester",
    type: SectionSubjectsDto,
  })
  @ApiResponse({ status: 404, description: "Plan not found" })
  async getSubjectsByPlan(
    @Query("plan") plan: string,
  ): Promise<SectionSubjectsDto> {
    if (!plan) {
      throw new NotFoundException("Plan parameter is required");
    }

    const organizedSubjects =
      await this.subjectPlanService.getSubjectsByPlanOrganized(plan);
